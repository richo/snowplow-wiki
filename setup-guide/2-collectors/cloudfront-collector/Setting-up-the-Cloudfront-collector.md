...
Click the **Create** button. The new bucket should now be visible on the list of buckets on the left of the screen. On selecting it, you wil get a warning that the bucket is empty. (We haven't added the tracking pixel yet!)
...
You can download a copy of the tracking pixel from the [SnowPlow Github repo](https://github.com/snowplow/snowplow/tree/master/2-collectors/cloudfront-collector/static). One convenient way to quickly grab `ice.png` is to execute the following at the command line:
...
**Note:** in the screenshot above and the following screenshots the tracking pixel is listed as `i` rather than `ice.png`. This is because we plan to move from using `ice.png` to `i` in a near-term release. The two tracking pixels are identical, but using `i` enables us to use more characters for query string parameters, to pass data into the Cloudfront collector. When the upgrade is made, this guide will be updated accordingly, so that all references to `ice.png` are updated to `i`.
...
In the drop down you should see the bucket you setup in [step 1](#bucket1) that contains the tracking pixel `ice.png`. Selet this - the **Origin ID** field should be automatically populated for you:
...
Write down the **Subdomain Name** for the distribution you just created. (Highlighted above - in our case it is `http://dzvb5g6uxbzaz.cloudfront.net`.) You will need this in the next step (to test the collector iw working), and when you setup your [tracker](choosing-a-tracker).
